...
T
```
Once that is done, you can start the marking process with the following command:
```lisp
CL-USER> (mark-assignments submissions-dir is-zipped grades-export-dir test-cases-dir)
```
where 
1. `submissions-dir` is the location for the folder or zip file that holds the folders for student's submissions.
...
3. `grades-export-dir` is the location for the D2L csv grades export folder.
4. `test-cases-dir` is the location for the test cases lisp file.
---
### Output
...
# Possible Issues
- Improper tail recursive functions of the format 
```lisp
(defun bad-recursive () 
  (bad-recursive))
```
are not handled by the AUTO-MARKER. This is because this function is not doing anything illegal (due to tail recursion support, you will never get a stack overflow), so no error is raised.
# Credits
The auto-marker was created by [Nabil NY Mansour](https://github.com/NabilNYMansour) under the supervision of [Professor Marcus Santos](https://github.com/marcus3santos). 
