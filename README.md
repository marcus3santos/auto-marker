...
5. `due-datetime` is a list representing the date and time the assignment is due, formatted as follows `(quote (<MMM> <DD> <TTTT> <PP>))`, where `<MMM>` are the three first letters of the name of the month (in upper or lower case), e.g., `sep`, `<DD>` is a number representing the month, e.g., `3` (or `03`), `<HHMM>` is a number with at most four digits representing the time, 2 digits for the hour, and 2 digits for minutes, e.g., `0145`, and `<PP>` is either `am` or `pm`. Here are examples of  valid due dates: `(quote (sep 1 0100 pm))`, `(quote (oct 23 1200 pm))`, `(quote (Dec 11 1200 pm))`
