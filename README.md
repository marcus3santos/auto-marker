...
5. `due-datetime` is a list representing the date and time the assignment is due, format as follows `(<MMM> <DD> <TTTT> <PP>`, where `<MMM>` are the three first characters of the name of the month, e.g., `sep`, `<DD>` is the they of the month, e.g., `3` (or `03`), `<TTTT>` is a (at most) four digits number representing the time, e.g., '145', and `<PP>` is either `am` or `pm`. Here are examples of  valid due dates `(quote (sep 1 1 pm))`, `(quote (oct 23 1200 pm))`, `(quote (Dec 11 12 pm))`
