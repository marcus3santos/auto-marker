...
5. `due-datetime` is a list representing the date and time the assignment is due.
...
2. A report file of format `/report.csv` that will have the same information as the D2L grades export csv file with the difference being that the grades are updated accordingly. So for the example above, `/report.csv` will be as follows:
  ...
  ```
  Note:
  1. If a student exists in the grades export but not in the submissions folder, then the grade in the `/report.csv` file would not be updated. So in this example, Alucard Vampireson and Timb Handerson did not submit a file, so their grade is not updated.
  2. Late assignment submissions reveive a zero score and a the following `Comment` in the log score: `Late assignemt submission`, or something to that effect.
