...
(defun mark-std-solution (student-solution test-cases-dir &optional (ws nil))
  ...
  (progn
    ...
    (list (format nil "~f" (calc-mark ws *results*)) ; (* (/ (get-num-correct-cases *results*) (get-total-cases *results*)) 100))
	  (if *endless-loop* "Endless Loop or runtime error" "OK")
	  (if *endless-loop*
	      (:= description (format nil "Endless loop or runtime error when evaluating the following assertion(s):~%~{- ~A~%~}" (reverse *endless-loop*)))
	      description)
