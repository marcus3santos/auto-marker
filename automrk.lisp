...
(defmacro check (&body forms)
  `(combine-results
     ,@(loop for f in forms collect `(report-result (time-execution ,f ,*max-time*) ',f))))
  ...
  (progn
    ...
    (handler-case (progn
      ...
      (serious-condition (e)
        ...
        (format t "~%> AUTO-MARKER: Error \"~a\" is found~%" description)
        (return-from mark-std-solution (list "0" "Runtime_Error" description (change-results-readable *results*) e))))
    (list (format nil "~f" (calc-mark ws *results*)) ; (* (/ (get-num-correct-cases *results*) (get-total-cases *results*)) 100))
	  (if *endless-loop* "Endless Loop" "OK")
	  (if *endless-loop*
	      (:= description (format nil "Endless loop when evaluating the following assertion(s):~%~{- ~A~%~}" (reverse *endless-loop*)))
	      description)
...
(form-date-time '(\"Nov 26 1999\" \"345 PM\")) returns '(NOV 26 345 PM)"
  (let ((date (car l))
	(time (cadr l)))
    (multiple-value-bind (md p1) (read-from-string date)
      (multiple-value-bind (tp p2) (read-from-string time)
	(list md (read-from-string (subseq date p1))
	      tp (read-from-string (subseq time p2)))))))
...
(defun >time (tp1 tp2)
  (let ((t1 (car tp1))
	...
	(p1 (cadr tp1))
	(p2 (cadr tp2)))
    (or (and (eql p1 'pm) (eql p2 'am))
	(and (eql p1 'am) (eql p2 'am)
	     (> t1 t2))
	(and (eql p1 'pm) (eql p2 'pm)
	     (> t1 t2)))))
      ...
      (fad:walk-directory submissions-choice
		...
		(progn
		  ...
		  (let* ((std-name (get-student-name (namestring pathname)))
			 ...
			 (std-solution-list (if (>date  formated-std-subm-dt due-date-time)
						(list "0"
						      "OK"
						      (format nil "Late submission. Assignment due at: ~a Submitted at: ~a~%" due-date-time std-submission-date-time)
						      nil
						      (format nil "Late submission. Assignment due at: ~a Submitted at: ~a~%" due-date-time std-submission-date-time))
